    ...
    private function _comfort_check($readings, $names) {
        ...
        foreach ($readings as $did => $sensors) {
            ...
            if (isset($sensors['co2'])) {
                $v = $sensors['co2']['value'];
                if ($v > 2000) {
                    $insights[] = $this->_make(self::CRITICAL, 'fas fa-exclamation-circle', $room . ' CO₂ is dangerously high (' . round($v) . ' µg/m³) — ventilate immediately', 'Comfort');
                    $all_comfortable = false;
                } elseif ($v > 1000) {
                    $insights[] = $this->_make(self::WARNING, 'fas fa-cloud', $room . ' CO₂ is elevated (' . round($v) . ' µg/m³) — room needs ventilation', 'Comfort');
                    $all_comfortable = false;
                }
            }
