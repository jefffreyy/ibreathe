...
<body>
    ...
    <div class="form-wrapper">
        <div class="form-container">
            <h4 class="text-center mb-3">Disable Seat Assignment</h4>
            <form id="disabledSeatForm" method="post" action="<?= site_url('admin/disable_seat/' . $seat_id) ?>">
            <div class="mb-3">
                    <label for="seat_id" class="form-label">Seat ID</label>
                    <input type="number" class="form-control" id="seat_id" name="seat_id" value="<?= htmlspecialchars($seat_id) ?>" required readonly>
                </div>
                <div class="mb-3">
                    <label for="date_disabled" class="form-label">Date Disabled</label>
                    <input type="date" class="form-control" id="date_disabled" name="date_disabled" required>
                </div>
                <div class="mb-3">
                    <!-- <label for="created_date" class="form-label">Created Date</label> -->
                    <input type="hidden" class="form-control" id="created_date" name="created_date" value="<?= date('Y-m-d\TH:i', strtotime($created_date)) ?>" required readonly>
                </div>
