...
<body>
    <div class="wrapper">
        ...
        <main class="content">
            <div class="mt-3 ms-3">
                <a href="<?= base_url('admin/dashboard') ?>" class="btn btn-outline-dark btn-sm">← Home</a>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 col-lg-2">
                        <div class="list-group">
                            <a href="<?= base_url('admin/enable_seats') ?>" class="list-group-item list-group-item-action active bg-warning border-0 fw-bold text-dark">Disabled Seats</a>
                        </div>
                    </div>

                    <!-- Disabled Seats Content -->
                    <div class="col-md-9 col-lg-10">
                        <?php if (!empty($disabled_seats)) : ?>
                            <?php foreach ($disabled_seats as $seat) : ?>
                                <div class="reservation-entry d-flex justify-content-between align-items-center mb-2 p-3 border rounded">
                                    <div>
                                        <strong>Seat #<?= htmlspecialchars($seat['seat_id']); ?></strong><br>
                                        <small>Disabled on: <?= date('m/d/Y', strtotime($seat['date_disabled'])); ?></small>
                                    </div>
                                    <form method="post" action="<?= base_url('admin/enable_seat/' . $seat['id']); ?>">
                                        <button class="btn btn-success btn-sm" type="submit">Enable</button>
                                    </form>
                                </div>
                            <?php endforeach; ?>
                        <?php else : ?>
                            <p>No disabled seats exist.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
